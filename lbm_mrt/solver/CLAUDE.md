...
> **Phase 2 状态**: 🔧 运行时网格已实施 (35 内核 × get_nx_active/get_ny_active) | 2026-07-17

该目录是整个项目的计算核心，编译产出**三个**独立二进制文件（`mcmp_sim` / `mcmp_sim_hydrate` / `mcmp_huang_unified`）。Python 层仅负责调用，不应修改此处的任何 `.cu` / `.h` 文件逻辑，除非有明确的物理或数值需求。
